...
def process_crypto_analysis(
    ...
    for file in all_files:
        ...
        # ----- 新增：每个币种日收益率 vs 市值、vs 换手率（volume/marketCap） -----
        # 注：df 中已有 volume_to_marketcap 列，为避免混淆，再显式创建一个 turnover 列
        df['turnover'] = np.where(df['marketCap'] > 0, df['volume'] / df['marketCap'], np.nan)
        valid = df[['daily_return', 'marketCap', 'turnover']].dropna()
        if len(valid) >= 5:   # 至少5个有效交易日
    ...
    with pd.ExcelWriter(overview_file, engine="openpyxl") as writer:
        ...
        if not daily_corr_df.empty:
            ...
            pd.DataFrame([{'币种':'','样本数':0,'Spearman相关系数':np.nan,'p值':np.nan,'结论':'无数据'}]).to_excel(writer, sheet_name="日度相关性", index=False)
    ...
    with pd.ExcelWriter(mutation_file, engine="openpyxl") as writer:
        if not mutation_df.empty:
            ...
            pd.DataFrame(columns=['结束时间','起始时间','币种','涨跌幅(%)','当前价格','当期市值']).to_excel(writer, sheet_name="突变报告", index=False)
    ...
    metric_cols = ['平均日收益率', '波动率(涨跌烈度)', '平均市值', '平均交易量']
    available_metrics = [col for col in metric_cols if col in long_df.columns and long_df[col].notna().sum() >= 3]
    cross_corr_matrix = None
    if len(available_metrics) >= 2:
        cross_corr_matrix = long_df[available_metrics].corr(method='spearman')
    ...
    for file in all_files:
        ...
        df['date'] = df['timeOpen'].dt.date
        df['daily_return'] = df['close'].pct_change()
        df = df[['date', 'daily_return', 'marketCap', 'volume']].dropna()
        if not df.empty:
            all_panel_data.append(df.assign(币种=coin_name))

    daily_corr_summary = []
    if all_panel_data:
        panel_df = pd.concat(all_panel_data, ignore_index=True)
        for feat in ['marketCap', 'volume']:
            sub = panel_df[['daily_return', feat]].dropna()
            if len(sub) >= 10:
                r, p = spearmanr(sub['daily_return'], sub[feat])
                daily_corr_summary.append({
                    '指标': '日收益率 vs ' + feat,
                    'Spearman相关系数': r,
                    'p值': p,
                    '结论': '相关' if p < 0.05 else '不显著相关'
                })
            else:
                daily_corr_summary.append({
                    '指标': '日收益率 vs ' + feat,
        ...
        daily_corr_summary.append({'指标': '无数据', 'Spearman相关系数': np.nan, 'p值': np.nan, '结论': '无数据'})

    adv_file = "crypto_advanced_correlations.xlsx"
    with pd.ExcelWriter(adv_file, engine="openpyxl") as writer:
        long_df.to_excel(writer, sheet_name="长期指标明细", index=False)
        if cross_corr_matrix is not None:
            cross_corr_matrix.to_excel(writer, sheet_name="长期指标相关矩阵")
        else:
            pd.DataFrame().to_excel(writer, sheet_name="长期指标相关矩阵", index=False)
        pd.DataFrame(daily_corr_summary).to_excel(writer, sheet_name="日收益率与市值交易量相关", index=False)

    # ============ 新增：每个币种自身相关性输出（日收益率 vs 市值、换手率）============
    coin_specific_file = "crypto_coin_specific_correlations.xlsx"
    coin_specific_df = pd.DataFrame(coin_specific_corrs).sort_values('币种')
    with pd.ExcelWriter(coin_specific_file, engine="openpyxl") as writer:
    ...
    print(f"3. 已生成高级相关性分析: {adv_file}")
    print(f"4. 已生成单币种相关性分析: {coin_specific_file}")
